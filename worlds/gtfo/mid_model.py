...
"""
The prefix `MID_` is added to most classes in this file to prevent naming conflicts
"""
...
class LazyObject:
    ...
    @classmethod
    def convert(typ: Type[T], value: Any):
        """
    ...
    def get_value(self, typ: Type[T], *keys: str) -> Optional[T]:
        """
        ...
        for key in keys:
            if key in self.source:
                return LazyObject.convert(type, self.source[key])
        else:
            raise KeyError(f"Failed to find key(s): {keys}")
            return None

    def get_list(self, type: Type[T], *keys: str) -> List[T]:
        """
        Look up the first matching key in `source`, then convert the value to `typ`.
        ...
        result = []
        for value in values:
            result.append(LazyObject.convert(type, value))
        return result
...
class Mid_Region(LazyObject):
    ...
    def get_connected_paths(self) -> List[int]:
        return self.get_list(int, "ConnectedPaths")

    def get_connected_locations(self) -> List[int]:
        return self.get_list(int, "ConnectedLocations")
...
class Mid_Path(LazyObject):
    ...
    def get_starting_region(self) -> int:
        return self.get_starting_region(int, "StartingRegion")

    def get_ending_region(self) -> int:
        return self.get_ending_region(int, "EndingRegion")
...
class Mid_Item(LazyObject):
    ...
    def get_path_reqs(self) -> Mid_ReqItem:
        ...
        """
        self.get_value(Mid_ReqItem, "PathReqs")

    def get_required_expedition(self) -> str:
        """
        Ties this item to a particular expedition.
        Prevents it from entering the pool unless that expedition is selected.
        """
        return self.get_value(str, "RequiredExpeditionName")
...
class Mid_GameData(LazyObject):
    ...
    def get_expeditions(self) -> List[Mid_ExpeditionData]: 
        return self.get_list(str, "Expeditions")
